...
export function totalOf(expenses: Expense[]): number {
  return expenses.reduce((n, e) => n + (Number.isFinite(e.amount) ? e.amount : 0), 0);
}
...
export function byCategory(expenses: Expense[]): CategoryTotal[] {
  return EXPENSE_CATEGORIES.map((category) => {
    ...
    return { category, total: totalOf(rows), count: rows.length };
  })
    .filter((c) => c.count > 0)
...
export function useExpenses() {
  ...
  return { expenses, loaded, add, update, remove };
}
